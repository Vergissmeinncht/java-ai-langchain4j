...
public class RAGTest {
    ...
    public void testReadDocument(){
        Document document1 = FileSystemDocumentLoader.loadDocument("src/main/resources/测试.txt");
        System.out.println(document1.text());

        // 加载单个文档
        Document document2 = FileSystemDocumentLoader.loadDocument("src/main/resources/knowledge/医院信息.txt", new
                TextDocumentParser());
// 从一个目录中加载所有文档
        List<Document> documents1 = FileSystemDocumentLoader.loadDocuments("src/main/resources/knowledge", new
                TextDocumentParser());
// 从一个目录中加载所有的.txt文档
...
//并使用默认的文档解析器对文档进行解析(TextDocumentParser)
        Document document = FileSystemDocumentLoader.loadDocument("src/main/resources/knowledge/医院信息.txt");
...
//查看向量数据库内容
    ...
    public void testDocumentSplitter(){
        ...
        //自定义文档分割器
        //按段落分，每段不超过300个token，重叠部分30个token
        ...
        //按字符计算
        //DocumentByParagraphSplitter documentByParagraphSplitter = new DocumentByParagraphSplitter(300, 300);
     ...
     * 1个英文字符大约为0.3个token
     * 1个中文字符大约为0.6个token
     * 每个模型的token计算方式不同，以上只是一个大致的估算，实际情况可能会有所不同
     */
    @Test
    public void testDocumentSplitter2(){
        String text = "这是一个测试文本，用于测试文档分割器的token计算。";
        UserMessage userMessage = UserMessage.userMessage(text);


        QwenTokenizer dashScopeApiKey = new QwenTokenizer(System.getenv("DASH_SCOPE_API_KEY"), "qwen-max");
        HuggingFaceTokenizer huggingFaceTokenizer = new HuggingFaceTokenizer();
        int tokenCount = dashScopeApiKey.estimateTokenCountInMessage(userMessage);
